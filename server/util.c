...
/*
 * Eseguito in risposta ad un comando #<number_channel>.
...
void send_destroy(int socket, char *chat_name) {
	char msg[MSG_SIZE]; int i, temp, ret;
...
void send_channels(int socket) {
    ...
    for (i = 0; i < desc; i++) {
        char tmp[MSG_SIZE];
        sprintf(tmp, "%s (%d), ", channel[i]->name, channel[i]->descriptor);
        strcat(msg, tmp);
    }
