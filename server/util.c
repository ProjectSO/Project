...
void broadcast(msg_t* msg) {
    ...
    for (i = 0; i < current_users; i++) {
        if (strcmp(msg->nickname, users[i]->nickname) != 0) {
            ...
            //enqueue(GLOBAL_CHAT, msg_to_send);
            users[i]->rcvd_msgs++;
        } else {
            users[i]->sent_msgs++;
        }
...
/*
 * Inoltra un messaggio di un utente a tutti gli altri utenti nel canale.
 *
 * Nel caso in cui il messaggio sia originato da SERVER_NICKNAME, esso
 * viene inviato a tutti gli utenti connessi.
 */
void broadcast_channel(msg_t* msg) {
...
void end_chat_session_bis(int socket, char *address, const char *msg) {
    ...
    printf("bis");
    end_chat_session_for_closed_socket_bis(socket, address);
}
...
/*
 * Metodo eseguito dal thread che deve processare la coda dei messaggi del canale.
 */
 
void* broadcast_channel_routine(void *args) {
    while (1) {
        msg_t *msg = dequeue_channel();
...
void *send_create(session_thread_args_t* args, char *chat_name, char *nickname) {
    ...
    pthread_t broadcast_channel_thread;
    ret = pthread_create(&broadcast_channel_thread, NULL, broadcast_channel_routine, NULL);
    PTHREAD_ERROR_HELPER(ret, "Errore nella creazione di un thread");

    ret = pthread_detach(broadcast_channel_thread);
    ...
    do {
        ...
        if (len > 0) {
            ...
            if (msg[0] == COMMAND_CHAR) {
                ...
                //qui fare in modo che mess arrivi agli utenti del canale
                enqueue_channel(nickname, msg);
            }
        } else if (len < 0) {
...
void send_destroy(int socket, char *chat_name) {
	char msg[MSG_SIZE]; int i, temp, ret, j;
	...
	for(i = 0;i < desc; i++) {
		if(strcmp(channel[i]->name,chat_name) == 0) {
			...
			//printf("elimino canale %s", channel[i]->name); 
			//free(channel[i]);
			...
			for(j=0; j<channel[i]->num; j++){
				if(strcmp(channel[i]->users_list[j]->nickname, chat_name) == 0)
					...
					send_leave(channel[i]->users_list[j]->socket, channel[i]->users_list[j]->nickname, channel[i]->descriptor);
					channel[i]->users_list[j]->leave = 1;
				}
			} 
	...
	sprintf(msg, "Il canale %s e' stato distrutto", chat_name);	
	send_msg_by_server(socket, msg);
}

void send_leave(int socket, const char *nickname, unsigned int descriptor) {
	char msg[MSG_SIZE];int i, j, ret;
	...
	for (j=0; j < channel[descriptor]->num; j++){
		if(strcmp(channel[descriptor]->users_list[j]->nickname, nickname) == 0)
			//free(channel[descriptor]->users_list[j]);
			break;
	}
