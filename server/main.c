...
sem_t user_data_sem;
unsigned int desc;
extern channel_t* channel[MAX_CHANNEL];
int quit = 0;
...
void *chat_session(void *arg) {
    ...
    size_t join_msg_len = recv_msg(args, msg, MSG_SIZE);
    if (join_msg_len < 0) {
        ...
        if (len > 0) {
            ...
            if (msg[0] == COMMAND_CHAR) {
                if (strcmp(msg + 1, LIST_COMMAND) == 0) {
                    ...
                    send_help(args->socket);
                } else if (strcmp(msg + 1, CREATE_COMMAND) == 0) {
					printf("Ricevuto comando create channel dall' utente %s\n", nickname);
					send_create(args, nickname, nickname);
			    } else if (isdigit(msg[1])) {
					int i,j;
					if(isdigit(msg[2])) {
						i = (msg[1]) - '0';
						j = (msg[2]) - '0';
						...
						i = (msg[1]) - '0';
					printf("Ricevuto comando join per il canale %d dall' utente %s\n", i, nickname);
					if( i >= desc || ((msg[1] == '0') && isdigit(msg[2]))){
						sprintf(error_msg, "Errore, canale non esistente");
						send_msg_by_server(args->socket, error_msg);
					}
					else send_join(args, nickname, i);
			    } else if (strcmp(msg + 1, CHANNELS_COMMAND) == 0) {
					printf("Ricevuto comando channels dall'utente %s\n", nickname);
					send_channels(args->socket);			
                } else {
...
/*
 * gestione SIGINT (crtl+c) e SIGQUIT (ctrl+\)
 */
void  SIGhandler(int sig){
     ...
     printf(" Vuoi davvero uscire? [s/n] ");
     I:
     c = getchar();
     if (c == 's' || c == 'S'){
          //notifca agli utenti la chiusura del server e chiude l' utente
          quit = 1;
          for(i = current_users-1; i >= 0; i--){
			  sprintf(error_msg, "Il server e' stato chiuso,grazie per aver partecipato alla chatroom!!!");
			  send_msg_by_server(users[i]->socket, error_msg);
			  int ret = close(users[i]->socket);
...
/*
 * gestione chiusura terminale (SIGHUP)
 */
void SIGHUPhandler(int sig){
	//notifca agli utenti la chiusura del server e chiude l' utente
	char error_msg[MSG_SIZE]; int i;
    for(i = current_users-1; i >= 0; i--){
		sprintf(error_msg, "Il server e' stato chiuso,grazie per aver partecipato alla chatroom!!!");
		send_msg_by_server(users[i]->socket, error_msg);
		int ret = close(users[i]->socket);
...
int main(int argc, char* argv[]) {
    ...
    fprintf(stderr, "Server della chatroom avviato\n");
    ...
    hup.sa_flags = SA_RESTART; // Restart functions if interrupted by handler 
    
    struct sigaction time;
    time.sa_handler = gestione_timeout;
    sigemptyset(&time.sa_mask);
    time.sa_flags = SA_RESTART; // Restart functions if interrupted by handler 
    
    if (sigaction(SIGINT, &sa, NULL) == -1)
        perror("Errore nella gestione di SIGINT");
    
    if (sigaction(SIGQUIT, &sa, NULL) == -1)
        perror("Errore nella gestione di SIGQUIT");
    
    if (sigaction(SIGTSTP, &sa, NULL) == -1)
        perror("Errore nella gestione di SIGTSTP");
        
    if (sigaction(SIGHUP, &hup, NULL) == -1)
        perror("Errore nella gestione di SIGHUP");
    
    if (sigaction(SIGALRM, &time, NULL) == -1)
        perror("Errore nella gestione di SIG_ALRM"); 
    ...
    // avvio thread per il broadcast dei messaggi
