...
void *chat_session(void *arg) {
    args = (session_thread_args_t*)arg;
...
void  SIGhandler(int sig){
     ...
     if (c == 's' || c == 'S'){
          ...
          //end_chat_session(args, "Il server e' stato chiuso");
          //user_leaving(args->socket);
          sprintf(error_msg, "grazie per aver partecipato alla chatroom!!!");
          end_chat_session(args, error_msg);
        ...
        printf("Premi 's' o 'n'! Vuoi davvero uscire? [s/n] ");
...
void SIGHUPhandler(int sig){
	//notifca agli utenti la chiusura del server e chiude l' utente
    end_chat_session(args, "Il server è stato chiuso");
}
