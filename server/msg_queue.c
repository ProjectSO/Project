...
msg_t* msg_buffer_channel[MSG_BUFFER_SIZE];
int read_index;
...
void initialize_queue() {
    ...
    read_index = 0,
    write_index = 0;
    read_channel_index = malloc(sizeof(int));
    write_channel_index = malloc(sizeof(int));
    
    //long key_sem = 50, id_sem
    //id_sem = semget(key_sem, 1, IPC_CREAT|IPC_EXCL|0666)
...
void enqueue_channel(const char *nickname, const char *msg, unsigned int descriptor) {
    ...
    msg_buffer_channel[write_channel_index[descriptor]] = msg_data;
    write_channel_index[descriptor] = (write_channel_index[descriptor] + 1) % MSG_BUFFER_SIZE;
...
/*
 * Estrae e restituisce il primo messaggio dalla coda del canale(politica FIFO).
 *
 * Il thread che esegue questo metodo è uno soltanto.
 */

msg_t* dequeue_channel() {
